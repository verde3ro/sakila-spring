...
// IMPORTANTE: `authority` es una URL absoluta (puerto 8080), distinta al
// origen del frontend (puerto 3000). oidc-client-ts usa este valor para
// hacer discovery (`/.well-known/openid-configuration`) y luego llama
// directamente a los endpoints absolutos que ese documento reporte
// (authorize, token, jwks, etc.). Ninguna de esas llamadas pasa por el
// proxy de vite.config.js, así que el Spring Authorization Server debe
...
const userManagerConfig = {
	...
	accessTokenExpiringNotificationTimeInSeconds: 60, // dispara la renovación 60s antes de expirar
	pkce: true,
	code_challenge_method: 'S256',
	userStore: new WebStorageStateStore({ store: window.localStorage }),
...
// Log opcional para depurar el silent renew mientras lo pruebas
userManager.events.addSilentRenewError((err) => {
	console.error('Silent renew falló:', err);
});
...
export const getAccessToken = async () => {
	...
	// Evita mandar al resource server un token ya vencido
